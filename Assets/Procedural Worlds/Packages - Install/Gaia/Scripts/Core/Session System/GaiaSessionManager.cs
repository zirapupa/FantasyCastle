...
namespace Gaia
{
    ...
    public class GaiaSessionManager : MonoBehaviour
    {
        ...
        private IEnumerator ExecuteUpdateWorld(GaiaOperation op)
        {
            ...
            //First Save the project - there might be newly created terrains where the terrain data file is not saved yet
            EditorApplication.ExecuteMenuItem("File/Save Project");

            //if we are not in a terrain loading scenario already - it could be that the user supplied terrains where the terrain data is instanced
            //"serialized with the scene" rather than stored as separate objects.
            if (!GaiaUtils.HasDynamicLoadedTerrains())
            {
                if (Terrain.activeTerrain != null)
                {
                    string path = GaiaDirectories.GetTerrainDataScenePath();

                    foreach (Terrain t in Terrain.activeTerrains)
                    {
                        if (string.IsNullOrEmpty(AssetDatabase.GetAssetPath(t.terrainData)))
                        {
                            AssetDatabase.CreateAsset(t.terrainData, $"{path}/{t.name}.asset");
                        }
                    }
                }
            }
